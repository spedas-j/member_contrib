...
; Check the arguments
;
if undefined(binarr) or undefined(arrs2bin) or undefined(minval) or undefined(maxval) or $
  undefined(binwidth) then begin
  dprint, 'In bin1D, some arguments are missing!'
  return
endif
...
for mtharr=0L,marrays-1L do begin
  for nthbin=0L,nbins-1L do begin
    if (kinbin(nthbin) ne 0) then begin
      averages(nthbin,mtharr)=total(arrs2bin(iinbin(nindxstarts(nthbin):nindxends(nthbin)-1),mtharr),nan=nan)/kinbin(nthbin); same as using mean()
      if (kinbin(nthbin) gt 1) then begin
        stdevs(nthbin,mtharr)=stddev(arrs2bin(iinbin(nindxstarts(nthbin):nindxends(nthbin)-1),mtharr),nan=nan)
