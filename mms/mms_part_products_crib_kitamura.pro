...
;
;
PRO mms_part_products_crib_kitamura,trange=trange,probe=probe,load_dfg=load_dfg,dfg_ql=dfg_ql,load_fpi=load_fpi,load_state=load_state,erange=erange,parange=parange,gyrorange=gyrorange,ion=ion,outputs=outputs,no_update=no_update,fac_type=fac_type,regrid=redrid
  ...
  level='def' ; 'pred'
  if not undefined(load_state) then mms_load_state,probes=probe,level=level,no_download=no_update
  ...
  if not undefined(load_fpi) then mms_load_fpi,probe=probe,trange=trange,data_rate='brst',level='l1b',datatype='d'+species+'s-dist',no_update=no_update
  ...
  for i=0,n_elements(outputs)-1 do begin
    mms_part_products,name,mag_name=bname,pos_name=pos_name,trange=trange,outputs=outputs,energy=erange,pitch=parange,gyro=gyrorange,regrid=regrid,fac_type=fac_type,datagap=datagap
