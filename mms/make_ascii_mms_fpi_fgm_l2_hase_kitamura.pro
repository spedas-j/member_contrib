  ...
  for i=0,n_elements(probes)-1 do begin
    ...
    if undefined(no_load_state) then begin
      if undefined(no_update) then begin
        mms_load_state,trange=[trange[0]-600.d,trange[1]+600.d],probes=probes[i],datatypes=['spinras','spindec']
      endif else begin
        mms_load_state,trange=[trange[0]-600.d,trange[1]+600.d],probes=probes[i],datatypes=['spinras','spindec'],/no_download
      endelse
      mms_load_mec,trange=[trange[0]-600.d,trange[1]+600.d],probes=probes[i],no_update=no_update
    endif
